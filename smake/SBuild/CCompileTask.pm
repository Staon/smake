...
# Ctor
#   Usage: newCCompileTask($name, $resource, \@target, \@source, \@deps)
sub newTask {
	$class = shift;
...
sub getTaskCommand {
	...
	my $options = $_[5];
	return $profile->getToolChain->getCCompiler($targets, $sources, $options);
}
...
sub getOptions {
	my $profile = $_[1];
	my $reporter = $_[2];
	my $cflags = $profile->getProfileStack->getOptions("CFLAGS", $profile, $reporter);
	my $cppflags = $profile->getProfileStack->getOptions("CPPFLAGS", $profile, $reporter);
