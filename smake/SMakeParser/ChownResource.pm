...
#  Change file owner tail resource
package SMakeParser::ChownResource;

use SMakeParser::TailResource;

@ISA = qw(SMakeParser::TailResource);

use SBuild::ChownTask;

#  Ctor
#
#  Usage: newResource($id, $srcresid, $owner [, $group])
sub newResource {
	my $class = $_[0];
...
sub processTailResource {
	...
	my $task = SBuild::ChownTask->newTask(
					$this->getID, $this,
					$resource, $this->{owner}, $this->{group});
