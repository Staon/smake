...
#  ODBC link profile
package SMakeParser::SoapODBCProfile;
...
sub changeProject {
	...
	# -- remove libraries of conflicted profiles
	$assembler->removeLink(["qnxodbc.lib"]);
	$assembler->removeSysLink(["dblib", "odbc"]);
	# -- add libraries
	$assembler->addLink(["avesoapodbc.lib", "dbwatch_cli.lib", "aodbc.lib"]);
	$assembler->addForceLink(["aodbc_asoap.lib"]);
	$assembler->addSysLink(["socket_s"]);
