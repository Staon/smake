...
# ACAP compilation task
package SMakeParser::AcapTask;
...
sub getTaskCommand {
	...
	# -- utility path
	my $path = SMakeParser::ProfileUtils::getFilePath($profile, "acap_compile", "acap_compile");

	# -- compose the command
	return "$path -o " . $targets->[0]->getFullname($profile) . " " . $sources->[0]->getFullname($profile);
}
