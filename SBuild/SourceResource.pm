...
#  Resource of a source file
package SBuild::SourceResource;
...
sub getFullname {
	...
	return $profile->getDirEngine->getSourceFile($filename);
}
...
sub getAbsolute {
	...
	my $filename = $this->getFilename;
	return $profile->getDirEngine->getSourceFileAbsolute($filename);
}
...
sub getDirectory {
	my $this = $_[0];
	my $profile = $_[1];
	return $profile->getDirEngine->getSourcePath
}
