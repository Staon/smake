...
sub getTaskCommand {
	...
	# -- get function prefix
	my $fceprefix = $profile->getProfileStack->getOptions("BISON_PREFIX", $profile, $reporter);
	$fceprefix = "-p $fceprefix" if($fceprefix);
	
	return "bison -v $fceprefix -o $targets->[0] -d $sources->[0]";
}
